...
double U, V, R, ftemp, kinetic_energy, U2, V2, UV, Fx, Fy, CX, CY, A, B, C;
...
void initial_state()
{
    ...
    for(int y=0; y<ny; y++)
		{
      JJ = (double)y/(double)(ny-1);
      II = (double)x/(double)(nx-1);
      id = x*ny+y;
			R = rho[id] = rho0;
			if(y<ny/2)
				U = u[id] = v0*tanh(lambda*(JJ-0.25));
			else
				U = u[id] = v0*tanh(lambda*(0.75-JJ));
      V = v[id] = epsilon*v0*sin(2.*M_PI*(II+0.25));
			C = -1.5*(U*U+V*V);
			for(int k=0; k<np;k++)
			{
        A = U*cx[k]+V*cy[k];
        B = 4.5*A*A;
        f1[id*np+k] = wf[k]*R*(1.+3.*A+B+C);
			}
		}
}
///----------------------------------------------------------------------------------------------------------------------------------
    ...
    for(int y=0; y<ny; y++)
      ...
      u[id] = U;
			...
			for(int k=0; k<np; k++)
			{
				A = U*cx[k]+V*cy[k];
				B = 4.5*A*A;
				f1[id*np+k] = omega1*f1[id*np+k]+omega*wf[k]*R*(1.+3.*A+B+C);
				newx = x+cx[k];
				newy = y+cy[k];
				if(x==0 || x==nx-1)
					newx = (newx+nx)%nx;
